...
@app.function(secrets=[modal.Secret.from_name("Ewt4oEgyDDiC0WbyLqcI")])
def download_dataset(config: DatasetConfig):
    import os

    from roboflow import Roboflow

    rf = Roboflow(api_key="Ewt4oEgyDDiC0WbyLqcI")
    project = (
...
def main(quick_check: bool = True, inference_only: bool = False):
    ...
    birds = DatasetConfig(
        workspace_id="practices-mjfv8",
        project_id="farm_animal_detection",
        version=1,
        format="yolov9",
        target_class="🐥",
    )
