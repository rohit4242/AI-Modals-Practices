...
def convert_and_save_hf(args):
    ...
    try:
        ...
        # Handle LLaMA 3.1 specific rope_scaling configuration
        if hasattr(hf_config, 'rope_scaling') and isinstance(hf_config.rope_scaling, dict):
            if 'rope_type' in hf_config.rope_scaling and hf_config.rope_scaling['rope_type'] == 'llama3':
                # Convert LLaMA 3.1 rope_scaling to the expected format
        ...
        logger.warning(f"AutoConfig cannot load the huggingface config: {str(e)}")
        hf_config = None
